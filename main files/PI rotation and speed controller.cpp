...
void set_rotation(string input){
    ...
    if(rotations > -1000 && rotations < 1000){
        ...
        if (evt.status == osEventMail) {
            ...
            switch(mail->type){
                ...
                case 6: //Prints current hash count
                    ...
                    break;
                case 7:
                    pc.printf("Max Velocity: %d Target Position: %.3f Current Position: %.3\r\n", mail->int_package, mail->float_package_2, mail->float_package);
                    break;
                case 8:
                    pc.printf("Rotation Torque: %.3f Velocity Torque: %0.3f, Final Torque: %d\r\n", mail->float_package, mail->float_package_2, mail->int_package);
                    break;
                default:
...
void motor_controller_thread(){
    ...
    while(1){
        ...
        position_differential = (position_error - prev_position_error)*1000/position_change_rate.read_ms();
        position_change_rate.reset();
        prev_position_error = position_error;
        float T_r = (k_pr*position_error) + (k_dr * position_differential);
        /*****************************/
        
        /**** Speed Controller Here ****/
        velocity = (current_pos - last_pos) * 10 / 6;
        float velocity_error = max_speed - velocity;
        velocity_error_integral += velocity_error;
        float T_s = ((velocity_error * k_ps) + (velocity_error_integral * k_is));
        T_s = position_differential < 0 ? -T_s : T_s;
        /*****************************/
        float T;
        if(position_differential < 0){
            T = T_s > T_r ? T_s : T_r;
        }else{
            T = T_s < T_r ? T_s : T_r;
        }
