...
Thread motor_cntrl_thread(osPriorityNormal, 1024);
/************************************************************************/
//Mutexes
...
void positionChange(){
    ...
    if (intState > intStateOld) {
        stateCount += 1; //assumes positive rotation
        intStateOld = intState;
        motorOut((intState-orState+lead+6)%6); //+6 to make sure the remainder is positive
    }
} 

void mailbox_add(uint32_t type, uint32_t int_package = 0, float float_package = 0.0){
        ...
        if (evt.status == osEventMail) {
            ...
            switch(mail->type){
                ...
                case 6: //Prints current hash count
                    ...
                    break;
                case 7:
                    pc.printf("Velocity: %i Torque: %.3f\r\n", mail->int_package, mail->float_package);
